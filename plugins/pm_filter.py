...
async def pm_search(client, message):
    ...
    if ispm:
        ...
        text=f"<b>🏴‍☠️Ahhoyy Pirate.🎃 {message.from_user.mention} ,\n\n💢Sorry But you Need to Search Movies in our Grp🗿\n🔆UnderStand...!🔆</b>",
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        "💀Join Grp or Die.🗿", url=link
                    )
                ]
            ]
        ),
...
async def year_search(client: Client, query: CallbackQuery):
    ...
    if n_offset== '':
        btn.append(
            [InlineKeyboardButton(text="♨Page Finished♨", callback_data="buttons")]
        )
    elif n_offset == 0:
        btn.append(
...
async def advantage_spoll_choker(bot, query):
    ...
    await query.answer('❗Checking for Movie in Piracy World .♻..')
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        btn= [[
            InlineKeyboardButton("WatchOnline", url=online),
            InlineKeyboardButton("FastDownload", url=download)
        ],[
